...
public class Java8CommonProgrammingQA {
	...
	public static void main(String[] args) {
		...
		Set<String> deptNamesInSet = studentList.stream().map(Student::getDept).collect(Collectors.toSet());
		...
		Map<String, Optional<Student>> stdMap = studentList.stream().collect(
		...
		Student student = studentList.stream().sorted(Comparator.comparing(Student::getRank)).skip(2).findFirst().get();
